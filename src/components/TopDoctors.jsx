    ...
    <div className="  flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10">
      ...
      <div className=" w-full grid grid-cols-auto pt-4 gap-4 gap-y-6 px-3 sm:px-0 ">
        {doctors.slice(0, 10).map((item,index) => {
          return(
            <div  key={item.id} onClick={()=>{navigate(`/appointment/${item._id}`)}} className="border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-8px] transition-all duration-500" >
      ...
      <button onClick={()=>{navigate("/doctors");scrollTo(0,0)}} className="cursor-pointer bg-blue-50 text-gray-600 px-12 py-3 rounded-full mt-12 font-medium hover:scale-104 transition-transform duration-500 ease-out transform-gpu">more</button>
    </div>
  );
};
